...
package scaleform.clik.controls 
{
    ...
    // Constants:
		protected static const AXIS_INPUT_DELAY:Number = 300; // W3+-
		protected static const AXIS_INPUT_REPEAT_DELAY:Number = 120; //W3+-
    ...
    // Protected Properties:
		...
		// Timer value for processing joystick scrolling manually
		protected var _lastJoyUpdateTime:int = 0; // W3+-
    ...
    // Public Methods:
        ...
        override public function handleInput(event:InputEvent):void {
            ...
            if (event.handled) { return; }
				...
				var elapsedTime:Number = curTime - _lastJoyUpdateTime;
				
				if ((curTime - _lastJoyUpdateTime) > (AXIS_INPUT_DELAY * 2))
				{
					_lastJoyUpdateDir = "none";
				}
				...
				if (details.code == KeyCode.PAD_RIGHT_STICK_AXIS)
				{
					...
					if (direction.code == KeyCode.UP && 
						(elapsedTime > AXIS_INPUT_DELAY ||
						(_lastJoyUpdateDir == "uprepeat" && elapsedTime > AXIS_INPUT_REPEAT_DELAY)))
					{
						if (position == 1) { return; }
                        position = Math.max(1, position - 1);
                        event.handled = true;
						scrolled = true;
						if (_lastJoyUpdateDir == "up")
						{
							_lastJoyUpdateDir = "uprepeat";
						}
						else if (_lastJoyUpdateDir != "uprepeat")
						{
							_lastJoyUpdateDir = "up";
						}
						
						trace("GFX - _lastJoyUpdateDir: ", _lastJoyUpdateDir);
					}
					else if (direction.code == KeyCode.DOWN  && 
						(elapsedTime > AXIS_INPUT_DELAY ||
						(_lastJoyUpdateDir == "downrepeat" && elapsedTime > AXIS_INPUT_REPEAT_DELAY)))
					{
						if (position == _maxScroll) { return; }
                        position = Math.min(_maxScroll, position + 1);
                        event.handled = true;
						scrolled = true;
						if (_lastJoyUpdateDir == "down")
						{
							_lastJoyUpdateDir = "downrepeat";
						}
						else if (_lastJoyUpdateDir != "downrepeat")
						{
							_lastJoyUpdateDir = "down";
						}
					}
					
					if (scrolled)
					{
						_lastJoyUpdateTime = curTime;
					}
				}
